...
describe('Block Quote', function () {
    ...
    it('should not allow empty block quotes', function () {
        var doc = new index_1.Document();
        var quote = doc.blockQuote();
        chai_1.expect(function () { return quote.toJSON(); }).to.throw();
    });
    it('should create valid block quote', function () {
        var doc = new index_1.Document();
        var quote = doc.blockQuote();
        quote.paragraph().text('quoted');
        chai_1.expect(doc).to.be.validADF();
        chai_1.expect(quote.toJSON()).to.deep.equal({
            type: 'blockquote',
            content: [
                {
                    content: [
